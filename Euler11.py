def adjacentProduct(numDigits, source):
    ans = [-1]
    ans += [0] * numDigits
    temp = [0]
    temp += [0] * numDigits
    ...
    for row in range(len(source)):
        for col in range(len(source[0])):
            #down
            if row < len(source) - numDigits + 1:
                temp[0] = 1
                for i in range(numDigits):
                    temp[i + 1] = source[row + i][col]
                    temp[0] *= temp[i + 1]
                if temp[0] > ans[0]:
            ...
            #right
            if col < len(source) - numDigits + 1:
                temp[0] = 1
                for i in range(numDigits):
                    temp[i + 1] = source[row][col + i]
                    temp[0] *= temp[i + 1]
                if temp[0] > ans[0]:
                    for j in range(len(ans)):
                        ans[j] = temp[j]
            ...
            #diagdl
            if row < len(source) - numDigits + 1 and col > numDigits - 2:
                temp[0] = 1
                for i in range(numDigits):
                    temp[i + 1] = source[row + i][col - i]
                    temp[0] *= temp[i + 1]
                if temp[0] > ans[0]:
                    for j in range(len(ans)):
                        ans[j] = temp[j]
            ...
            #diagdr
            if row < len(source) - numDigits + 1 and col < len(source) - numDigits + 1:
                temp[0] = 1
                for i in range(numDigits):
                    temp[i + 1] = source[row + i][col + i]
                    temp[0] *= temp[i + 1]
